---
layout: default
title: Política de Privacidad
permalink: /es/privacy-policy/
lang: es
---

# Política de Privacidad de Jaksimsamil (v1.0)

**Fecha de entrada en vigor:** 20 de julio de 2026

## 1. Disposiciones generales

Jaksimsamil protege la información personal de los usuarios de conformidad con la legislación aplicable.

## 2. Información recopilada

| Categoría | Información | Finalidad | Conservación |
|---|---|---|---|
| Iniciar sesión con Apple | Identificador de Apple, correo electrónico | Autenticación | Hasta la eliminación de la cuenta |
| Firebase Authentication | UID de Firebase | Autenticación | Hasta la eliminación de la cuenta |
| Firestore | Hábitos, horarios, configuraciones y registros | Prestación del servicio | Hasta la eliminación de la cuenta |
| Información del dispositivo | SO y versión de la app | Mejora del servicio | Hasta cumplir la finalidad |

## 3. Finalidad del uso

- Inicio de sesión
- Sincronización de datos
- Restauración de compras
- Operación del servicio
- Publicidad mediante Google AdMob

## 4. Publicidad

La aplicación utiliza Google AdMob. En iOS, los anuncios personalizados solo se muestran con el consentimiento del usuario mediante ATT.

## 5. Proveedores

| Proveedor | Finalidad |
|---|---|
| Google LLC | Firebase y AdMob |
| Apple Inc. | Inicio de sesión con Apple y compras integradas |

## 6. Transferencia internacional

La información puede procesarse en servidores operados por Google y Apple fuera del país.

## 7. Conservación y eliminación

Los datos personales se eliminan cuando el usuario elimina su cuenta.

## 8. Derechos del usuario

El usuario puede solicitar acceso, corrección o eliminación de su información personal.

## 9. Seguridad

Se utilizan HTTPS, Firebase Security Rules y controles de acceso.

## 10. Contacto

- Email: demianjun1@gmail.com

## 11. Cambios

Esta política podrá modificarse cuando sea necesario.
